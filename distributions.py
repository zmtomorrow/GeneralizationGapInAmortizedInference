...
def batch_logistic_local_linear_logp(params,x):
    """
        x: [n,p_num,c_num,patch_dim,patch_dim]
        params: [n, fea_dim, x_dim, x_dim ]

    """
    b_size,p_num,c_num,p_dim,_=x.size()
    new_c_num=p_num*c_num

    params=params.view(b_size,90,-1)
    x=x.view(b_size,new_c_num,-1)
    means=params[:,:new_c_num,:]
    log_scales=params[:,new_c_num:2*new_c_num,:]
    linear=off_diag_matrix(params[:,2*new_c_num:,:].permute(0,2,1).contiguous().view(-1,66))
    means+=torch.bmm(linear,x.permute(0,2,1).contiguous().view(-1,new_c_num,1)).view(b_size,-1,new_c_num).permute(0,2,1).contiguous()
    centered_x = x - means
    inv_stdv = torch.exp(-torch.clamp(log_scales,min=-7.))
    cdf_plus = torch.sigmoid(inv_stdv * (centered_x + 1. / 255.))
    cdf_min = torch.sigmoid(inv_stdv * centered_x )
    cdf_plus=torch.where(x > 0.999, torch.ones(1).to(x.device),cdf_plus)
    cdf_min=torch.where(x < 0.001, torch.zeros(1).to(x.device),cdf_min)
    return torch.sum(torch.log(cdf_plus-cdf_min+1e-7),(1,2)).mean()


def discretized_mix_logistic_logp(x, l, alpha=0.0001):
    x = x.permute(0, 2, 3, 1)   
    l = l.permute(0, 2, 3, 1)
...
if __name__ == "__main__":
    x=torch.randn(100,4,3,16,16)
    params=torch.randn(100,90,16,16)
    print(batch_logistic_local_linear_logp(params,x))
